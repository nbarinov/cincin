...
describe('Toaster actions', () => {
  ...
  function clickAction(card: HTMLElement, index = 0): void {
    fireEvent.click(getActions(card)[index]!);
  }
...
describe('Toaster position', () => {
  ...
  it('should default to the bottom-left corner under RTL', () => {
    ...
    // still before the render call returns.
    const region = getRegion();
    expect(region.dataset.y).toBe('bottom');
...
describe('Toaster offset', () => {
  it('should publish the prop on the skin channel', () => {
    ...
    // own unit territory.
    render(() => (
      <Toaster toaster={createToaster<ToastContent>()} offset={56} />
    ));
