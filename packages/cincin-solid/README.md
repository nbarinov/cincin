<img src="https://raw.githubusercontent.com/nbarinov/cincin/v0.2.0/.github/assets/hero-solid.png" alt="cincin-solid 🥂">
...
headless building blocks under `cincin-solid/core`.

## Install

```bash
npm install cincin-solid
```
...
import { usePresenter, useToasts } from 'cincin-solid/core';

function Region(props) {
  // The toaster is read once, like a query client: remount to switch.
  // The exit clock finishes leaving toasts on time: declare your exit
  // animation's length, no transitionend listeners needed.
  const presenter = usePresenter(props.toaster, { max: 5, exitDuration: 400 });
  const toasts = useToasts(presenter);

  return (
    <ol>
      <For each={toasts()}>
        {(toast) => <Card toast={toast} presenter={presenter} />}
      </For>
    </ol>
  );
}
...
```tsx
import { useToastSwipe } from 'cincin-solid/core';
...
function Card(props) {
  ...
  return (
    <li
      ...
      {...swipe.handlers()}
    >
      {String(props.toast.entry.content)}
    </li>
  );
}
```

One Solid-specific note on that `<For>`: it keys items by reference,
and the presenter updates a toast by replacing its object, so a phase
flip re-creates the card's DOM node above. That is fine until you
animate; a skin that transitions its cards should key by `toast.key`
instead (a keyed helper, or a map over keys with a lookup), which is
exactly how the bundled Toaster renders.
...
import { Toaster } from 'cincin-solid';
...
`<ToasterProvider>` with context-aware `useToaster` and
`useToastEntries`.
