<img src="https://raw.githubusercontent.com/nbarinov/cincin/v0.3.0/.github/assets/hero-vue.png" alt="cincin-vue 🥂">
...
Swiping follows the corner. The default `swipe-directions` set is the
...
explicit `swipe-directions` overrides the pairing.
...
handlers to bind on the list with `v-on`;
...
import { Toaster } from 'cincin-vue';
...
typed content payload, `createToasterContext<MyContent>()` returns
`provideToaster` with injection-aware `useToaster` and
`useToastEntries`.
